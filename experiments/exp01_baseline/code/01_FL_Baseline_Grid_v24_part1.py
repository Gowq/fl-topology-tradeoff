#!/usr/bin/env python3
# 01_FL_Baseline_Grid_v24_part1.py
# PART 1 of 2: Split for 24h grid limit
# Includes: CIFAR-10 Horizontal + OPPORTUNITY Horizontal Cross-Device (Early/Inter/Late, Eps=[0,1,5,8])
# Features:
...
def run_vertical_fl(fusion_mode, dp_epsilon, seed, num_rounds=NUM_ROUNDS, epochs=LOCAL_EPOCHS):
    ...
    # v24: Enhanced early stopping tracking
    best_f1 = 0.0
    rounds_without_improvement = 0
    prev_loss = 0.0
    ...
    for round_idx in range(num_rounds):
        ...
        # Track F1 improvement for patience-based early stopping
        if f1 > best_f1:
            best_f1 = f1
            rounds_without_improvement = 0
        else:
...
def run_smoke_test():
    ...
    configs = [
        {'dataset': DATASET_CIFAR10, 'topology': 'horizontal', 'fusion': 'Horizontal', 'epsilon': 1.0},
        {'dataset': DATASET_OPPORTUNITY, 'topology': 'horizontal', 'fusion': 'Intermediate', 'epsilon': 1.0}
    ]
...
if __name__ == "__main__":
    ...
    with open('results/exp01_v24_results_part1.json', 'w') as f: json.dump(all_results, f)
    print("\nPART 1 DONE. Results saved to results/exp01_v24_results_part1.json")
