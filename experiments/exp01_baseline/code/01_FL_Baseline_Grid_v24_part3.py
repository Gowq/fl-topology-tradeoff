#!/usr/bin/env python3
# 01_FL_Baseline_Grid_v24_part3.py
# PART 3 of 3: Additional epsilon exploration
# Includes: ALL Horizontal + Vertical FL experiments with Eps=3.0
# Features:
# - v24: Checkpoint restoration fix, No LSTM (DP compatible), Multi-epsilon testing
...
DP_EPSILONS = [3.0]  # v24 part3: Test epsilon=3.0 across all configurations
...
def run_horizontal_fl(dataset_name, fusion_mode, dp_epsilon, seed, num_rounds=NUM_ROUNDS, epochs=LOCAL_EPOCHS):
    ...
    # Early stopping tracking (v24 fix: save best checkpoint)
    best_f1 = 0.0
    best_model_state = None  # v24: Save best checkpoint
    ...
    for round_idx in range(num_rounds):
       ...
       # Track improvement for patience-based early stopping (v24 fix)
        if f1 > best_f1:
            best_f1 = f1
            best_model_state = copy.deepcopy(global_model.state_dict())  # v24: Save best checkpoint
            rounds_without_improvement = 0
        else:
    ...
    if best_model_state is not None:
        ...
        # Re-evaluate with best model to get final metrics
        acc, f1, preds = evaluate(global_model, test_loader, DEVICE)
...
def run_vertical_fl(fusion_mode, dp_epsilon, seed, num_rounds=NUM_ROUNDS, epochs=LOCAL_EPOCHS):
    ...
    fusion_head = IntermediateFusionVFL(dropout_p=params['dropout']) if fusion_mode == 'Intermediate' else LateFusionVFL()
...
def run_smoke_test():
    print("\n" + "="*60)
    print(">> PRE-FLIGHT SMOKE TEST (1 Round, 1 Epoch) - Horizontal + Vertical")
    print("="*60)
    
    configs = [
        {'dataset': DATASET_CIFAR10, 'topology': 'horizontal', 'fusion': 'Horizontal', 'epsilon': 3.0},
        {'dataset': DATASET_OPPORTUNITY, 'topology': 'horizontal', 'fusion': 'Intermediate', 'epsilon': 3.0},
        {'dataset': DATASET_OPPORTUNITY, 'topology': 'vertical', 'fusion': 'Intermediate', 'epsilon': 3.0}
    ]
    
    for cfg in configs:
        print(f"Testing: {cfg['dataset']} {cfg['topology'].capitalize()} {cfg['fusion']} Eps={cfg['epsilon']}", end=' ')
        try:
            if cfg['topology'] == 'horizontal':
                run_horizontal_fl(cfg['dataset'], cfg['fusion'], cfg['epsilon'], 42, 1, 1)
            else:
...
if __name__ == "__main__":
    ...
    # 2. Main Experiments - PART 3: ALL Horizontal + Vertical with Eps=3.0
    experiments = []
    
    # CIFAR-10 Horizontal (1 config)
    experiments.extend([{'dataset': DATASET_CIFAR10, 'topo': 'horizontal', 'fusion': 'Horizontal', 'eps': eps} for eps in DP_EPSILONS])
    
    # OPPORTUNITY Horizontal: Early/Intermediate/Late fusion (3 configs)
    experiments.extend([{'dataset': DATASET_OPPORTUNITY, 'topo': 'horizontal', 'fusion': 'Early', 'eps': eps} for eps in DP_EPSILONS])
    experiments.extend([{'dataset': DATASET_OPPORTUNITY, 'topo': 'horizontal', 'fusion': 'Intermediate', 'eps': eps} for eps in DP_EPSILONS])
    experiments.extend([{'dataset': DATASET_OPPORTUNITY, 'topo': 'horizontal', 'fusion': 'Late', 'eps': eps} for eps in DP_EPSILONS])
    
    # OPPORTUNITY Vertical: Intermediate/Late fusion (2 configs)
    ...
    print(f"\n=== PART 3: ALL Horizontal + Vertical FL with Eps={DP_EPSILONS} v24 - Running {len(experiments)} experiment configurations ===")
    ...
    with open('results/exp01_v24_results_part3.json', 'w') as f: json.dump(all_results, f)
    print("\nPART 3 DONE. Results saved to results/exp01_v24_results_part3.json")
