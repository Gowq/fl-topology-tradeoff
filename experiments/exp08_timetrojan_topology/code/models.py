"""Exp. 08 models: topology-matched victims and centralized attack surrogate."""
...
class DeepConvEncoder(nn.Module):
    """DeepConvEncoder family used by Exp. 06, adapted for a single branch."""
...
class CentralSurrogate(nn.Module):
    def __init__(self, channels: int, num_classes: int, dropout: float = 0.1):
        super().__init__()
        self.encoder = DeepConvEncoder(channels, dropout)
...
class EightPartyFusion(nn.Module):
    """Same model graph in HFL and VFL; only ownership/training differs."""
