...
#SBATCH --array=0-2051%4
#SBATCH --requeue
#SBATCH --output=logs/exp07_corrected_%A_%a.out
#SBATCH --error=logs/exp07_corrected_%A_%a.err
...
if ! command -v nvidia-smi >/dev/null || ! nvidia-smi -L >/dev/null; then
    echo "CUDA preflight failed (driver) before Exp. 07 config ${SLURM_ARRAY_TASK_ID}" >&2
    exit 42
fi
if ! python3 -c 'import torch, sys; sys.exit(0 if torch.cuda.is_available() else 1)'; then
    echo "CUDA preflight failed (torch) before Exp. 07 config ${SLURM_ARRAY_TASK_ID}" >&2
    exit 42
fi
